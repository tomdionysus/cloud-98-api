...
describe('Logger', () => {
  ...
  it('should parse log level New', () => {
    const x1 = new Logger({ logLevel: 'error' })
  ...
  describe('debug', () => {
    it('should log debug when logLevel is equal', () => {
      const x2 = { log: () => {} }
      spyOn(x2, 'log')
      const x1 = new Logger({ logLevel: 'debug', logTo: x2 })
      x1.debug('TEST', 'ONE')
    ...
    it('should not log debug when logLevel greater', () => {
      const x2 = { log: () => {} }
      spyOn(x2, 'log')
      const x1 = new Logger({ logLevel: 'info', logTo: x2 })
      x1.debug('TEST', 'ONE')
  ...
  describe('info', () => {
    ...
    it('should not log info when logLevel greater', () => {
      const x2 = { log: () => {} }
      spyOn(x2, 'log')
      const x1 = new Logger({ logLevel: 'warn', logTo: x2 })
      x1.info('TEST', 'ONE')
      expect(x2.log).not.toHaveBeenCalled()
    })
  })
  ...
  describe('warn', () => {
    ...
    it('should not log warn when logLevel greater', () => {
      const x2 = { log: () => {} }
      spyOn(x2, 'log')
      const x1 = new Logger({ logLevel: 'error', logTo: x2 })
      x1.warn('TEST', 'ONE')
      expect(x2.log).not.toHaveBeenCalled()
    })
  })
