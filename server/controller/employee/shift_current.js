...
module.exports = {
  // GET /v1/employee/shift/current

  // Return Upcoming Shifts (Start > Now()) assigned to this user and associated organisations, premises, currency
  ...
  get (req, res, next) {
    ...
    async.series([
      (cb) => {
        let sql = 'SELECT `shift`.*, `premises`.`organisation_id` FROM `shift`, `premises` WHERE `premises`.`id` = `shift`.`premises_id` AND `shift`.`status`="accepted" AND `start_utc` > NOW() AND `shift`.`user_id`=?' 
